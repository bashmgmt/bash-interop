...
`tests/proofs.rs`, over the public API only. Each spawns real bash to cover
one mechanism that cannot be checked by reading the source — everything that
can be is left to the compiler.
...
| `the_prelude_is_non_invasive_and_self_reliant` | no `eval`, `trap`, `export`, `shopt`, no `set -x`-style option change, no variable outside `__BC__*`; no tool's bash exports a name, over the composed prelude each one actually runs in; a client's own trap and `IFS` are unaffected |
...
the generated text, over the composed prelude rather than over one tool's
fragment, since composed is what a subject sources.
