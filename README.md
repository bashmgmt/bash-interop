...
Start with `KB/onboarding.md`, then the crate doc (`cargo doc --open`) —
`rig`'s module doc carries a complete worked example. `KB/` holds the full
reference: the wire, the shell's account, the stack walk, the measured facts
the transport stands on.
